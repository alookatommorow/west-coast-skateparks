$(document).on('turbolinks:load', function () {
  if ($('body').hasClass('skateparks') && $('body').hasClass('index')) {
    var pageCount = 1;
    ...
    function infiniteScroll() {
      var scrolledToBottom =
        ...
        $(window).scrollTop();
      var morePagesLeft =
        pageCount < $('[data-total-pages]').data('total-pages');
    ...
    function updatePageNumber() {
      currentStateLink = currentStateLink.replace(
        /page=(\d+)/g,
        function (match, page) {
    ...
    function trackStateLinkClick(link) {
      if (window.analytics) {
        var state =
          $(link).data('state-link') || $(link).data('mobile-state-link');
      ...
      $(window).resize(function () {
        if (window.innerWidth > 767) {
          ...
          if (
            ...
            $('.parks-container').children().length > 1
          ) {
